...
TurnPeer 的 STUN 控制连接需要**认证**和 **AllocateTCP** 才能有效保活。

- 仅匿名 `SendBindingRequest()` 不足以维持连接——服务器会在 15~30s 空闲后主动关闭 TCP。
- `TURNServerAddr` + `Username`/`Password`/`Realm` 凭证认证后，调用 `AllocateTCP()` 创建 relay 分配。
- `TCPAllocation` 内部 `refreshAllocTimer` 每 `lifetime/2` 自动发 Refresh 请求（默认 5 分钟），保持连接活跃。
- `stunKeepalive`（9s ticker）作为健康检测，加上 `AllocateTCP` 的自动 Refresh 形成双重保活。
